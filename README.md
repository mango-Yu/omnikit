...
`tauri dev`、本地打包、GitHub Release 在系统里都是**不同身份**。开发时勾过的权限不会自动给 GitHub 安装包。从网盘/GitHub 下载后先去掉隔离属性：
...
1. 到「系统设置 → 隐私与安全性 → 录屏与系统录音」，用 **−** 删掉所有旧的 OmniKit
2. 只打开 **GitHub / Applications 里这一份**，点「开始录屏」，在弹窗里允许
3. 按 **Command+Q** 完全退出（只关窗口不够），再打开后才能真正录屏

没有 Apple 开发者证书时，每次重新下载安装包都可能要再授一次权。
